    ...
    case "settings":
      return (
        ...
        >
          ...
          <path d="M19.4 15a7.97 7.97 0 0 0 .1-1 7.97 7.97 0 0 0-.1-1l2-1.5-2-3.5-2.4 1a8 8 0 0 0-1.7-1L15 5h-6l-.3 2.6a8 8 0 0 0-1.7 1l-2.4-1-2 3.5 2 1.5a7.97 7.97 0 0 0-.1 1 7.97 7.97 0 0 0 .1 1l-2 1.5 2 3.5 2.4-1a8 8 0 0 0 1.7 1L9 19h6l.3-2.6a8 8 0 0 0 1.7-1l2.4 1 2-3.5-2-1.5z" />
        </svg>
      );
    case "menu":
      return (
...
export function AdminBadge({
  ...
  const classes =
    statusClass ?? palette[label] ?? "bg-slate-100 text-slate-600";

  return (
    <span
      className={`rounded-full px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.2em] ${classes}`}
    >
      {label}
    </span>
  );
      ...
      <div className="grid min-h-screen place-items-center bg-[linear-gradient(180deg,#eef2f7,#f8fafc)] text-slate-900">
        <div className="rounded-[2rem] bg-white px-6 py-8 text-center shadow-[0_20px_60px_rgba(15,23,42,0.08)]">
          ...
          <p className="mt-2 text-sm text-slate-600">
            Redirecting to sign in if this session is not an admin.
          </p>
        </div>
      </div>
    );
          ...
          <nav className="mt-8 space-y-2">
            {navItems.map((item) => (
              <Link
                key={item.label}
                href={item.href}
                className={`flex items-center gap-3 rounded-2xl px-4 py-3 text-sm font-medium transition ${activeHref === item.href ? "bg-[#0a2a78] text-white shadow-lg" : "text-slate-700 hover:bg-slate-50"}`}
              >
            ...
            <div className="flex items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex items-center gap-3 rounded-full bg-slate-50 px-3 py-2">
                  <div className="grid h-10 w-10 place-items-center rounded-full bg-[#0a2a78] text-white">
                    <AdminIcon name="user" />
                  </div>
            ...
            <nav className="mt-5 space-y-2">
              {navItems.map((item) => (
                <Link
                  key={item.label}
                  href={item.href}
                  className={`flex w-full items-center gap-3 rounded-2xl px-4 py-3 text-sm font-medium transition ${activeHref === item.href ? "bg-[#0a2a78] text-white" : "text-slate-700 hover:bg-slate-50"}`}
                >
                  <AdminIcon name={item.icon} />
                  {item.label}
                </Link>
