      ...
      <main className="mx-auto max-w-6xl px-4 py-10 md:px-8 md:py-14">
            ...
            <div className="absolute inset-x-0 bottom-0 p-6 md:p-10">
              <div className="grid gap-3 sm:grid-cols-3">
                ...
                <div className="rounded-[1.25rem] bg-white/10 p-4 backdrop-blur">
                  ...
                  <p className="mt-2 font-display text-2xl font-bold text-white">
                    {productsResponse.meta.totalPages}
                  </p>
                </div>
                <div className="rounded-[1.25rem] bg-white/10 p-4 backdrop-blur">
                  <p className="text-[11px] uppercase tracking-[0.24em] text-white/55">
        ...
        <section className="mt-12">
          <div className="mb-8 flex items-end justify-between gap-4">
            ...
            <span className="text-sm text-slate-400">
              Page {productsResponse.meta.currentPage} of{" "}
              {productsResponse.meta.totalPages}
            </span>
          </div>
          ...
          <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
            {productsResponse.products.map((product) => (
              <article
                key={product.id}
                className="overflow-hidden rounded-[1.75rem] border border-white/10 bg-[#0a1020] shadow-[0_16px_40px_rgba(0,0,0,0.18)]"
              >
        ...
        {productsResponse.meta.totalPages > 1 ? (
          <div className="mt-10 flex flex-wrap gap-3">
            {Array.from(
              { length: productsResponse.meta.totalPages },
              (_, index) => index + 1,
            ).map((pageNumber) => (
              <Link
                key={pageNumber}
