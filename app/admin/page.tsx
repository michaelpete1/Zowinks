...
const shortcuts = [
  {
    label: "Products",
    href: "/admin/products",
    description: "Add, edit, hide, or remove products.",
    icon: "layers" as const,
  },
  {
    label: "Categories",
    href: "/admin/categories",
    description: "Upload and organize product categories.",
    icon: "tag" as const,
  },
  {
    label: "Orders",
    href: "/admin/orders",
    description: "Track delivered and pending orders.",
    icon: "orders" as const,
  },
  {
    label: "Customers",
    href: "/admin/customers",
    description: "Review customer records and contact details.",
    icon: "contacts" as const,
  },
  {
    label: "Delivery",
    href: "/admin/delivery-methods",
    description: "Manage shipping methods and fees.",
    icon: "truck" as const,
  },
  {
    label: "App Settings",
    href: "/admin/settings/app",
    description: "Manage application configuration and branding.",
    icon: "settings" as const,
  },
  {
    label: "Profile",
    href: "/admin/settings",
    description: "Manage admin profile and credentials.",
    icon: "user" as const,
  },
];
...
export default function AdminDashboardPage() {
  ...
  const accessToken =
    ...
    (typeof window !== "undefined"
      ? window.localStorage.getItem("zowkins-admin-access-token")
      : null);
  ...
  useEffect(() => {
    ...
    const loadDashboard = async () => {
      ...
      if (productsResult.status === "fulfilled") {
        const products = extractArray<ProductDetails>(productsResult.value, [
          "products",
        ]);
      ...
      if (categoriesResult.status === "fulfilled") {
        const categories = extractArray<CategoryListItem>(
  ...
  return (
    <AdminShell
      ...
      subtitle="Live summary cards and route shortcuts for the admin backend."
    >
      {error ? (
      ...
      {loading ? (
        <p className="mb-6 text-sm text-slate-500">Loading dashboard data...</p>
      ) : null}
      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        {stats.map((stat) => (
          <article
            key={stat.label}
            className="rounded-[1.5rem] bg-white p-5 shadow-[0_14px_30px_rgba(15,23,42,0.06)]"
          >
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-2">
            {shortcuts.map((item) => (
              <Link
                key={item.label}
                href={item.href}
                className="group rounded-[1.5rem] border border-slate-100 bg-slate-50 p-5 transition hover:-translate-y-0.5 hover:border-slate-200 hover:bg-white"
              >
                <div className="flex items-center justify-between gap-3">
                  <div className="grid h-11 w-11 place-items-center rounded-2xl bg-[#0a2a78] text-white transition group-hover:bg-[#12386a]">
        ...
        <div className="space-y-6">
          <section className="rounded-[2rem] bg-[linear-gradient(180deg,#0a2a78_0%,#12386a_100%)] p-6 text-white shadow-[0_14px_30px_rgba(15,23,42,0.10)] md:p-8">
            ...
            <p className="mt-3 text-sm leading-6 text-slate-200">
              The overview now reflects live backend data for products,
          ...
          <section className="rounded-[2rem] bg-white p-6 shadow-[0_14px_30px_rgba(15,23,42,0.06)] md:p-8">
            ...
            <div className="mt-5 space-y-3 text-sm text-slate-600">
              <p>
                <strong className="text-slate-900">Products</strong> handles
                add, edit, hide, remove, and mark-new flows.
              </p>
              <p>
                <strong className="text-slate-900">Categories</strong> handles
                category uploads and grouping.
              </p>
              <p>
                <strong className="text-slate-900">Orders</strong> handles
                status updates and product line items.
              </p>
              <p>
                <strong className="text-slate-900">Customers</strong> holds
