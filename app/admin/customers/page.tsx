...
export default function CustomersPage() {
  ...
  const loadCustomers = async () => {
    ...
    try {
      const [customerListResponse, statsResponse, nonReferralListResponse] =
        await Promise.all([
          ...
          zowkinsApi.listAdminNonReferralCustomers(
      ...
      const nonReferralList: AdminCustomer[] = Array.isArray(
  ...
  const updateCustomer = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      await zowkinsApi.updateAdminCustomer(
        apiConnection.accessToken.trim(),
        selectedCustomer.id || (selectedCustomer as any)._id,
        {
      ...
      setError(
        err instanceof ApiError ? err.message : "Could not update customer.",
      );
    } finally {
      setSaving(false);
    }
  ...
  const removeCustomer = async (userId: string) => {
    ...
    try {
      await zowkinsApi.deleteAdminCustomer(
        apiConnection.accessToken.trim(),
        userId,
      );
      ...
      setError(
        err instanceof ApiError ? err.message : "Could not delete customer.",
      );
    } finally {
      setDeletingId("");
    }
  ...
  const openCustomer = async (customer: AdminCustomer) => {
    ...
    try {
      const data = await zowkinsApi.getAdminCustomer(
        apiConnection.accessToken.trim(),
        customer.id || (customer as any)._id,
      );
      ...
      <div className="grid gap-6 xl:grid-cols-[1.05fr_0.95fr]">
        <section className="rounded-[2rem] bg-white p-6 shadow-[0_14px_30px_rgba(15,23,42,0.06)] md:p-8">
          ...
          <div className="mt-4 rounded-[1.25rem] border border-cyan-100 bg-cyan-50 px-4 py-3 text-sm text-cyan-900">
            Non-referral customers:{" "}
            <span className="font-semibold">{nonReferralCount}</span>
          </div>
              ...
              >
                <div className="flex flex-wrap items-start justify-between gap-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    <span
                      className={`rounded-full px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.2em] ${customer.isReferralPartner ? "bg-cyan-50 text-cyan-700" : "bg-slate-100 text-slate-600"}`}
                    >
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  ...
                  <button
                    type="button"
                    onClick={() =>
                      void removeCustomer(customer.id || (customer as any)._id)
                    }
                    disabled={
        ...
        <div className="space-y-6">
          <section className="rounded-[2rem] bg-[linear-gradient(180deg,#0a2a78_0%,#12386a_100%)] p-6 text-white shadow-[0_14px_30px_rgba(15,23,42,0.10)] md:p-8">
            ...
            <div className="mt-5 space-y-3 text-sm leading-7 text-slate-200">
              <p>
                Save a bearer token once, then manage customers from this
                workspace.
              </p>
              <p>
                Filter by status or referral partner state and inspect full
          ...
          <section className="rounded-[2rem] bg-white p-6 shadow-[0_14px_30px_rgba(15,23,42,0.06)] md:p-8">
            ...
            <div className="mt-4 rounded-2xl bg-slate-50 px-4 py-4 text-sm leading-6 text-slate-600">
              <p>
                Customer records load automatically from your signed-in admin
                session.
              </p>
          ...
          <section className="rounded-[2rem] bg-white p-6 shadow-[0_14px_30px_rgba(15,23,42,0.06)] md:p-8">
            <p className="text-xs font-semibold uppercase tracking-[0.28em] text-slate-500">
              {selectedCustomer ? "Edit customer" : "Customer details"}
            </p>
            <h2 className="mt-2 font-display text-2xl font-bold text-slate-900">
              {selectedCustomer
