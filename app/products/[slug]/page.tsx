...
export default async function ProductPage({ params }: ProductPageProps) {
  ...
  const imageUrl = product.image?.url || "/desktop.jpg";
      ...
      <main className="mx-auto max-w-6xl px-4 py-10 md:px-8 md:py-14">
        <section className="overflow-hidden rounded-[2rem] border border-white/10 bg-[#0a1020] shadow-[0_20px_60px_rgba(0,0,0,0.22)] lg:grid lg:grid-cols-[1.02fr_0.98fr]">
          ...
          <div className="space-y-6 px-6 py-10 md:px-10 md:py-14 lg:px-12 lg:py-16">
            <div>
              <p className="text-xs uppercase tracking-[0.35em] text-white/55">{product.category}</p>
              <h1 className="mt-2 font-display text-4xl font-bold leading-tight text-white md:text-5xl">
                {product.name}
              </h1>
              <p className="mt-3 text-lg leading-7 text-slate-300">{product.description}</p>
            </div>

            <div className="grid gap-4 sm:grid-cols-2">
              <div className="rounded-[1.4rem] border border-white/10 bg-white/5 p-4">
                <p className="text-xs uppercase tracking-[0.24em] text-white/55">Price</p>
                <p className="mt-2 text-2xl font-bold text-white">{formatPrice(product.price)}</p>
              </div>
              <div className="rounded-[1.4rem] border border-white/10 bg-white/5 p-4">
                <p className="text-xs uppercase tracking-[0.24em] text-white/55">Stock</p>
                <p className="mt-2 text-2xl font-bold text-white">{product.inStock ? "In stock" : "Out of stock"}</p>
              </div>
            </div>
            ...
            <div className="grid gap-3 rounded-[1.5rem] border border-white/10 bg-[#081224] p-5 text-sm text-slate-300 sm:grid-cols-2">
              <div>
                <p className="text-xs uppercase tracking-[0.24em] text-white/45">Slug</p>
                <p className="mt-1 font-medium text-white">{product.slug}</p>
              </div>
              <div>
                <p className="text-xs uppercase tracking-[0.24em] text-white/45">Subcategory</p>
                <p className="mt-1 font-medium text-white">{product.subcategory || "N/A"}</p>
              </div>
              <div>
                <p className="text-xs uppercase tracking-[0.24em] text-white/45">Created</p>
                <p className="mt-1 font-medium text-white">{new Date(product.createdAt).toLocaleDateString()}</p>
              </div>
              <div>
                <p className="text-xs uppercase tracking-[0.24em] text-white/45">Updated</p>
                <p className="mt-1 font-medium text-white">{new Date(product.updatedAt).toLocaleDateString()}</p>
              </div>
            </div>
          </div>
        </section>
